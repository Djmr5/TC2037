...
labels = ['Expresión inválida', 'Real', 'Entero', 'Comentario', 'Variable', 'Asignación', 'Suma', 'Resta', 'Multiplicación', 'División', 'Potencia', 'Paréntesis que abre', 'Paréntesis que cierra', 'Espacios', 'Expresión inválida en']
                        ...
                        # =
                            ...
                            if index == 1:
                                if first_var == True:
                                    ...
                                    invalid_line.append(f"Expresión inválida en: {group}")
                                    index += 1
                            else:
                                    invalid_line.append(f"Expresión inválida en: {group}")
                                    index += 1
                            ...
                            if operator:
                                invalid_line.append(f"Expresión inválida operador en: {group}")
                                index += 1
                            else:
                                valid_line.append(f"{labels[group_idx]}: {group}")
                                index += 1
